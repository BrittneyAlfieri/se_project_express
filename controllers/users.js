...
const updateUser = (req, res) => {
  ...
  User.findByIdAndUpdate(
    ...
    .then((user) => {
      if (!user) {
        return res.status(NOTFOUND_ERROR.error).send({ message: "User not found" });
      }
    ...
    .catch((error) => {
      if (error.name === "ValidationError") {
        return res.status(INVALID_DATA_ERROR.error).send({ message: "Invalid data provided" });
      }
      
      res.status(DEFAULT_ERROR.error).send({ message: "An error has occurred on the server" });
    });
};
